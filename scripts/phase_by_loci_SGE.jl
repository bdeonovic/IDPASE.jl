...
function main()
  ...
  for file in readdir(parsed_args["out"])
    m = match(Regex("(REAL|SIM)::(\\w*)::(\\d*)::([\\w\\[\\]\\/\\.\\-]*)::($(join(name_comb,"|")))::([-+]?[0-9]*\.?[0-9]+).txt"),file)
    if m != nothing
      sim = m.captures[1]
      chr = m.captures[2]
  ...
  for (chr, lines) in real_completed
    for i in 1:length(lines)
      if !lines[i]
        ...
        #println(out,"bash $(parsed_args["base"])/launch_mcmc.sh $(parsed_args["in"])/$(parsed_args["prefix"])_true_$chr.txt $(parsed_args["in"])/$(parsed_args["prefix"])_reads_$chr.txt $(parsed_args["out"]) $i $chr x")
        println(out, "qsub -N REAL$(string(chr,"-",i)) $(parsed_args["base"])/launch_mcmc.sh $(parsed_args["in"])/out_true_$chr.txt $(parsed_args["in"])/out_reads_$chr.txt $(parsed_args["out"]) $i $chr x")
      end
    end
  end
