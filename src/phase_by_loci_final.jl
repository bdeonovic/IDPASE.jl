...
function main()
  ...
  #source_matrix = reshape(parsed_args["matrix"], length(source_names), 2)

  #num_src, num_combs = size(source_matrix)
  #num_src = size(source_matrix,1)

  
  println(STDERR,"Running MCMC...")
  #results = run_MCMC((data, parsed_args["iters"], parsed_args["burnin"], parsed_args["chains"], source_matrix))
  results = run_MCMC((data, parsed_args["iters"], parsed_args["burnin"], parsed_args["chains"]))
  ...
  for j in 1:num_combs, k in parsed_args["schedule"]
    ...
    if !use_src[2] && !use_src[3]
      continue
    end
    results = run_MCMC((data, parsed_args["iters"], parsed_args["burnin"], parsed_args["chains"], parsed_args["method"], use_src, k))
